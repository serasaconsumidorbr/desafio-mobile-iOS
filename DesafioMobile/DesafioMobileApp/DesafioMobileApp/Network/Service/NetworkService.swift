...
final class NetworkService {
    ...
    func fetch<T>(_:T.Type, endpoint: MarvelEndpoint, completion: @escaping(Result<T, Error>) -> Void) where T: Decodable {
        var urlRequest = endpoint.baseURL
        urlRequest.path.append(endpoint.path)
        
        guard let url = URL(string: "\(urlRequest)\(getCredentials())") else { return }
