...
final class HomeView: UIView {
    ...
    private lazy var heroTableView: UITableView = {
...
extension HomeView {
    func uptadeView() {
        ...
        self.heroTableView.hideSkeleton(transition: .crossDissolve(0.25))
        self.carroussel.hideSkeleton(transition: .crossDissolve(0.35))
    }
    
    func createSpinnerFooter()-> UIView{
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: heroTableView.frame.size.width, height: 100))
...
extension HomeView: UITableViewDelegate, SkeletonTableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if position > (heroTableView.contentSize.height-100-scrollView.frame.size.height){
            guard viewModel.loadingPage else { return }
            self.heroTableView.tableFooterView = createSpinnerFooter()
            self.carregaPersonagens()
        }
    }
    
    func carregaPersonagens(){
        self.viewModel.currentPage += 100
        self.viewModel.fetchHeros()
        self.heroTableView.tableFooterView = nil
    }
...
extension HomeView {
    ...
    private func buildScene() {
        ...
        contentView.addSubview(heroTableView)
    }
    ...
    private func heroListConstraints() {
        heroTableView.snp.makeConstraints { make in
